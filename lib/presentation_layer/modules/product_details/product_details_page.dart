...
import 'package:minhaloja/presentation_layer/modules/product_details/components/list_combos.dart';
  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<CartCubit, CartState>(
      ...
      listener: (context, cartState) {
        productCart = cartState.productListCart.firstWhereOrNull(
          (e) => e.productId.toString() == widget.product?.id.toString(),
        );
      },
                      ...
                      Padding(
                        ...
                        child: InkWell(
                          onTap: () => Share.share(
                            ...
                            subject: (widget.product ?? state.product!).name,
                          ),
                          child: CircleAvatar(
                            radius: 20,
                            backgroundColor:
                          ...
                          children: [
                            Text(
                              (widget.product ?? state.product!)
                                  ...
                                  .toCapitalized(),
                              style: design
                                  .h4(color: design.secondary100)
                                  .copyWith(fontWeight: FontWeight.w700),
                            ),
                            SizedBox(height: 4.height),
                            ListCombos(
                              combos: state.product?.combos ??
                                  widget.product!.combos,
                              comboSelected: productCart?.combo,
                              value: (item) {
                                setState(() {
                                  combo = item as ComboDTO;
                                  var productComboSelect = cartState
                                      .productListCart
                                      .firstWhereOrNull(
                                    (e) => (e.productId.toString() ==
                                            widget.product?.id.toString() &&
                                        (e.combo?.name == combo?.name)),
                                  );

                                  if (productComboSelect != null) {
                                    productCart = productComboSelect;
                                  }
                                });
                              },
                            ),
                            SizedBox(height: 4.height),
                            Text(
                              (widget.product ?? state.product!).description,
                            ...
                            if (productCart == null)
                              Padding(
                                padding:
                                    EdgeInsets.symmetric(vertical: 24.height),
                                child: Row(
                            ...
                            Row(
                              ...
                              children: [
                                ...
                                Text(
                                  'R\$${((combo?.value ?? (widget.product ?? state.product!).value)).formatWithCurrency().trim()}',
                                  style: design
                                      .h5(
